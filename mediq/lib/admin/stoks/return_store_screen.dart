// return_store_screen.dart – Fixed: sorting uses ValueNotifier (no full page refresh)
...
class _ReturnStoreScreenState extends State<ReturnStoreScreen>
  ...
  // Analytics tab filters & sort – now using ValueNotifier to avoid full rebuild
  ...
  // --- FIX: sort variables changed to ValueNotifier ---
  ...
  void dispose() {
    ...
    _sortColumnIndex.dispose(); // dispose notifiers
  ...
  // ──────────────────────────────────────────────────────────────────
  // PDF CERTIFICATE GENERATION (with charts) – identical to main store
  // ──────────────────────────────────────────────────────────────────
  Future<void> _generateCertificatePdf(
    ...
    try {
                  ...
                  child: pw.Column(
                    ...
                    children: [
                      ...
                      pw.Container(
                        ...
                        child: pw.Column(
                          ...
                          children: [
                            ...
                            pw.Text(
                              'The following Return Store stock inventory report has been generated and verified by the system administrator.',
                              style: const pw.TextStyle(
                      ...
                      pw.Row(
                        mainAxisAlignment: pw.MainAxisAlignment.spaceBetween,
                        children: [
                          pw.Text('Return Store Management System',
                              style: const pw.TextStyle(
      ...
      final fileName =
          'ReturnStore_Certificate_${DateFormat('yyyyMMdd_HHmm').format(now)}.pdf';
      ...
      await Share.shareXFiles(
        [XFile(file.path)],
        text: 'Return Store Inventory Certificate – $dateStr',
      );
      file.delete();
    } catch (e) {
      setState(() => _isGeneratingPdf = false);
  ...
  Future<void> _exportToCSV() async {
    ...
    try {
      ...
      final directory = await _getWritableDirectory();
      final file = File(
          '${directory.path}/return_store_export_${DateTime.now().millisecondsSinceEpoch}.csv');
      await file.writeAsString(csv);
      Navigator.pop(context);
      await Share.shareXFiles([XFile(file.path)],
          text: 'Return Store Stock Export');
      file.delete();
    } catch (e) {
      Navigator.pop(context);
  ...
  // ---------- Analytics: interactive modal for pie chart ----------
  ...
  // ---------- Analytics Tab (with RepaintBoundary for charts) – sort uses ValueNotifier ----------
        ...
        SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
                                ...
                                bottomTitles: AxisTitles(
                                  sideTitles: SideTitles(
                                    ...
                                    getTitlesWidget: (value, meta) {
                                      ...
                                      if (idx >= 0 && idx < topDrugs.length) {
                                        return Padding(
                                          padding: const EdgeInsets.only(top: 8),
                                          child: Transform.rotate(
                                            angle: -0.5,
                                            child: Text(
                                              topDrugs[idx].key.length > 12
                                                  ? '${topDrugs[idx].key.substring(0, 10)}..'
                                                  : topDrugs[idx].key,
                                              style: const TextStyle(
                                                  fontSize: 10,
              ...
              // Filter & sort controls – the sort button now updates ValueNotifier
              Card(
                elevation: 2,
                shape: RoundedRectangleBorder(
                ...
                child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      // Sort button – uses ValueListenableBuilder to change icon
              ...
              // ── Data Table – now nested with ValueListenableBuilder for sort ──
              ...
              child: StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, antibioticSnapshot) {
                  ...
                  if (antibioticSnapshot.hasError) {
                    return Center(
                        child: Text('Error: ${antibioticSnapshot.error}'));
                  }
