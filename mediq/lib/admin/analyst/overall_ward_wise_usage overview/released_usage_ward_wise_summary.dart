...
class _ReleasedUsageWardWiseSummaryScreenState
  ...
  // Helper to get category colour for the left border
  ...
  Future<void> _fetchWardData() async {
    ...
    try {
      ...
      // Store the category for each ward (to later assign left border)
      ...
      // Build convertible list with ward category
      List<Map<String, dynamic>> convertibleList = [];
      for (final entry in convertibleWardTotals.entries) {
        convertibleList.add({
          'wardName': entry.key,
          'quantity': entry.value,
          'category': wardCategoryMap[entry.key] ?? 'Other', // store category for left border
        });
      }
      ...
      // Build raw list (no category needed)
...
// ---------- Filter Panel for Ward Tab (updated with timezone-aware date pickers) ----------
