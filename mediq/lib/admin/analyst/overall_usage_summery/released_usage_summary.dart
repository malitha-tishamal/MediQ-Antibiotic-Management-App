...
class _ReleasedUsageSummaryScreenState
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      // Build list of WardSummary objects
      ...
      for (var entry in wardDrugUnits.entries) {
        ...
        final drugs = entry.value.entries
            .map((e) => DrugUsage(drugName: e.key, totalUnits: e.value))
            .toList()
          ..sort((a, b) => b.totalUnits.compareTo(a.totalUnits));
        summaries.add(WardSummary(wardName: wardName, drugs: drugs));
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Ward‑wise Releases Summary'),
      ...
      body: _isLoading
          ...
          : _wardSummaries.isEmpty
              ? const Center(child: Text('No release data found.'))
              : ListView.builder(
...
class DrugUsage {
  ...
  DrugUsage({required this.drugName, required this.totalUnits});
}
...
class WardSummary {
  ...
  WardSummary({required this.wardName, required this.drugs});
}